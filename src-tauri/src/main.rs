...
use crate::{aeskey::AES_KEY, decrypt::decrypt_to_memory};
...
#[tauri::command]
fn init_copywriting(app_handle: tauri::AppHandle, file_name: String) -> Result<CopywritingStruct, ErrorCode> {
    ...
    let key_bytes = base64::engine::general_purpose::STANDARD.decode(AES_KEY).map_err(|_| ErrorCode::AesKeyHasWrong)?;
    if key_bytes.len() != 32 {
    ...
    let (file_map, main_lua_file_name) = decrypt_to_memory(engine_path, key_slice).map_err(|_| ErrorCode::CannotDecryptData)?;
    let Some(main_lua_file_name) = main_lua_file_name else {return Err(ErrorCode::CannotDecryptData);};
...
fn find_all_game_file_name() -> Result<Vec<String>, ErrorCode> {
    let execute_dir = get_executable_file_path()
        .ok_or(ErrorCode::CannotGetExecuteDir)?;
    let mut engine_path = std::path::PathBuf::new();
    engine_path.push(execute_dir);
    engine_path.push("games");
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![init_copywriting, find_all_game_file_name])
