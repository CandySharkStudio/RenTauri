...
// 开始替换！
pub fn parse_embed(content: &str, files: &HashMap<String, String>) -> String {
    let mut result = Vec::new();
    // 遍历输出每一行！
    for line in content.lines() {
        if let Some(replaced) = try_parse_embed_line(line, files) {
...
fn find_word(s: &str, word: &str) -> Option<usize> {
    ...
    while i + word_bytes.len() <= bytes.len() {
        if &bytes[i..i + word_bytes.len()] == word_bytes {
            ...
            let next_ok = i + word_bytes.len() >= bytes.len()
